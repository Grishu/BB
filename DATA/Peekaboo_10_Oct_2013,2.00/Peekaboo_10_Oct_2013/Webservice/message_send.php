<?php
// http://192.168.1.196/PeekaBoo_Webservice/message_send.php?uid=75&receiver_uid=76,72,73,74
...
$sqlnew = "SELECT uuid( )";
	...
	if($fileSize>0){
		...
		$imageurl = "uploads/msgmedia/".$lastimgname;
		...
		$inserted = mysql_insert_id();		
             ...
             while ($i < count($receiver_uid)) {
				 ...
				 $sql = "INSERT INTO messages (image_name,isender_user_id,isender_Profile_Url,image_url,receiver_uid) values ('".$lastimgname."','".$uid."','".$ProfileUrl."','".$imageurl."','".$receiver_uid[$i]."')";
...
{
	$StatuS = false;
	$posts = array('status'=>$StatuS,'result'=>"Message Sending failes.");
}
