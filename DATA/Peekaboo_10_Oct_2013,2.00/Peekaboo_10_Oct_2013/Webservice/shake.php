<?php
//http://192.168.1.196/PeekaBoo_Webservice/shake.php?uid=76&latitude=73.181098&longitude=22.307309
include('config.php');
...
//echo $date;
//exit();
	$select_user = mysql_query("SELECT `user_id` FROM `shake` WHERE `user_id`=$uid");
	$num_rows = mysql_num_rows($select_user);
	
	if($num_rows>0){
		$update = mysql_query("UPDATE `shake` SET `latitude`=$latitude,`longitude`=$longitude,`curr_timestemp`='".$date."' WHERE `user_id`=$uid",$conn) or die(mysql_error());
	}else{
		$insert = mysql_query("INSERT INTO `shake` (`longitude`, `latitude`, `user_id`, `curr_timestemp`) VALUES ($longitude,$latitude,$uid,'".$date."')",$conn) or die(mysql_error());
	}
